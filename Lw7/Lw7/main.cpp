...
int main()
{
	...
	std::shared_ptr<IShape> e1 = std::make_shared<CEllipse>(MakeConstDPoint(0, 0), 100, 100);
	std::shared_ptr<IShape> e2= std::make_shared<CEllipse>(MakeConstDPoint(0, 0), 100, 100);
	std::shared_ptr<CGroupShape> b; //= std::make_shared<CGroupShape>(e1, e2);
	auto a = b->GetOutlineStyle();
	b->~CGroupShape();
	a->IsEnable();

	//std::shared_ptr<IOutlineStyle> a = std::make_shared<CGroupOutlineStyle>(b->GetGroup());
	...
	//shared_ptr<IShape> faceTwoGroup = make_shared<CGroupShape>(faceTwo);
